...
<svelte:head>
	<title>Fundraising & Disbursements Admin - CANFACS</title>
</svelte:head>
...
<section class="py-10 bg-slate-950 min-h-screen text-slate-100">
	<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
		...
		<div class="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-6">
			<div>
				<div class="flex items-center gap-2 mb-2">
					...
					<span class="text-xs font-semibold text-slate-400">Fundraising & Accountability</span>
				</div>
				<h1 class="text-3xl font-extrabold text-white tracking-tight">Fundraising & Disbursements</h1>
				<p class="text-slate-400 text-sm mt-1">
					Maintain campaign pledges, verified donor payments, and transparent public disbursements.
				</p>
			</div>
		...
		{#if form?.message}
			<div class="p-4 rounded-xl {form?.success ? 'bg-emerald-950/80 border-emerald-500/50 text-emerald-300' : 'bg-red-950/80 border-red-500/50 text-red-300'} border text-xs leading-relaxed flex items-center justify-between">
		...
		<div class="grid grid-cols-2 lg:grid-cols-5 gap-4">
			...
			<div class="bg-slate-900/80 p-5 rounded-2xl border border-emerald-900/40 shadow">
				...
				<div class="text-[11px] text-slate-400 mt-1">{data.stats.receivedCount} Verified Received</div>
			</div>
			...
			<div class="col-span-2 lg:col-span-1 bg-gradient-to-br from-slate-900 to-slate-950 p-5 rounded-2xl border border-slate-700 shadow">
				<div class="text-[11px] font-bold text-slate-300 uppercase tracking-wider">Net Available</div>
				<div class="text-2xl sm:text-3xl font-black text-white mt-1">
											...
											<td class="py-3.5 px-4 text-right whitespace-nowrap">
												<div class="flex items-center justify-end gap-2">
													...
													<form method="POST" action="?/deleteDonation" use:enhance onsubmit={(e) => {
														if (!confirm(`Are you sure you want to delete donation from ${don.donor_name}?`)) {
															e.preventDefault();
														}
													}}>
														<input type="hidden" name="id" value={don.id} />
		...
		<!-- Tab 2: Disbursements Ledger -->
		{#if activeTab === 'disbursements'}
			<div class="space-y-4">
				<div class="bg-slate-900 rounded-2xl border border-slate-800 overflow-hidden shadow">
					<div class="p-4 border-b border-slate-800 flex items-center justify-between">
						<div>
							<h3 class="font-bold text-white text-sm">Official Disbursements & Wire Transfers</h3>
							<p class="text-xs text-slate-400">Publicly auditable disbursements sent to on-the-ground recipients.</p>
						</div>
					</div>
